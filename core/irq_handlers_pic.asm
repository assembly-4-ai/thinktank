; irq_handlers_pic.asm - IRQ Stubs & Handler Registration for APIC (PIC-compliant)
...
extern get_acpi_lapic_base, get_acpi_ioapic_base  ; Use getter functions instead of direct globals
...
setup_irq_handlers:
    ...
    lea rdi, [rel irq_dispatch_table]
    mov rcx, 64
    lea rax, [rel default_irq_handler]
...
.init_loop:
    ...
    lea rsi, [rel keyboard_irq_handler]
    call register_irq_handler
    
    mov rdi, AHCI_IRQ
    lea rsi, [rel ahci_irq_handler]
    call register_irq_handler
    
    mov rdi, TIMER_IRQ
    lea rsi, [rel timer_irq_handler]
    call register_irq_handler
...
register_irq_handler:
    ...
    lea rax, [rel irq_dispatch_table]
    mov rcx, rdi
...
configure_lapic:
    ...
    ; Get LAPIC base address using getter function
    call get_acpi_lapic_base
    mov rbx, rax
    test rbx, rbx
...
configure_ioapic:
    ...
    ; Get IOAPIC base address using getter function
    call get_acpi_ioapic_base
    mov rbx, rax
    test rbx, rbx
    jz .done ; Skip if not available
...
irq_keyboard_stub:
    ...
    mov rdi, KEYBOARD_IRQ
    mov rsi, [rel irq_dispatch_table + KEYBOARD_IRQ * 8]
    call rsi
...
irq_ahci_stub:
    ...
    mov rdi, AHCI_IRQ
    mov rsi, [rel irq_dispatch_table + AHCI_IRQ * 8]
    call rsi
...
irq_timer_stub:
    ...
    mov rdi, TIMER_IRQ
    mov rsi, [rel irq_dispatch_table + TIMER_IRQ * 8]
    call rsi
